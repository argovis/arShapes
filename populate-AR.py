...
#for timestep in [1667]:
    ...
    for AR in ARs:
        try:
            ...
            AR['true_geolocation'] = copy.deepcopy(AR['geolocation'])
            AR['geolocation'] = add_noise(AR['geolocation'])
            if 'flags' in AR:
                AR['flags'].append('noise_added')
            else:
                AR['flags'] = ['noise_added']
            db.arShapes.insert_one(AR)
